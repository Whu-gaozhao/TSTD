...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'color_block/scannet_infos_train.pkl',
        pipeline=train_pipeline,
        classes=class_names,
        test_mode=False,
        ignore_index=len(class_names),
        scene_idxs=data_root + 'seg_info/block_resampled_scene_idxs.npy'),
    val=dict(
        type=dataset_type,
        data_root=data_root,
...
evaluation = dict(pipeline=eval_pipeline, interval=10, save_best='miou')
