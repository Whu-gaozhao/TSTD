...
from .transformer_sa_module import TransformerEncoderLayer
...
class PointFPModule(BaseModule):
    ...
    def __init__(self,
        ...
        self.attention = None
        if attention:
            self.attention = TransformerEncoderLayer(mlp_channels[1], 2, 256)
    ...
    def forward(self, target: torch.Tensor, source: torch.Tensor,
        ...
        new_features = self.mlps(new_features).squeeze(-1)
        if self.attention is not None:
            new_features = new_features.transpose(1, 2)
            new_features = self.attention(new_features).transpose(1, 2)
