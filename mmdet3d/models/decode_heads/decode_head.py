...
class Base3DDecodeHeadNew(BaseModule, metaclass=ABCMeta):
    """Base class for BaseDecodeHead.
    ...
    def __init__(self,
                 ...
                 ignore_index=255,
                 init_cfg=None):
        super(Base3DDecodeHeadNew, self).__init__(init_cfg=init_cfg)
        self.channels = channels
        self.num_classes = num_classes
        self.dropout_ratio = dropout_ratio
        self.conv_cfg = conv_cfg
    ...
    def forward_train(self, inputs, img_metas, pts_semantic_mask, train_cfg):
        ...
        # losses = self.losses(seg_logits, pts_semantic_mask)
        # return losses
        seg_logits, seg_logits_mid = self.forward(inputs,pts_semantic_mask)
        losses = self.losses(seg_logits, seg_logits_mid, pts_semantic_mask)
        return losses
    ...
    def cls_seg_mid(self, feat):
        """Classify each points."""
        if self.dropout is not None:
            feat = self.dropout(feat)
        output = self.conv_seg_mid(feat)
        return output

    @force_fp32(apply_to=('seg_logit', 'seg_logit_mid',))
    def losses(self, seg_logit, seg_logit_mid, seg_label):
        ...
        for loss_decode in losses_decode:
            if loss_decode.loss_name not in loss:
                ...
                loss[loss_decode.loss_name] += 0.4 * loss_decode(
                    seg_logit_mid,
                    seg_label,
                    weight=None,
                    ignore_index=self.ignore_index)                
